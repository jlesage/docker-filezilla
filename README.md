...
This project provides a Docker container for [FileZilla](https://filezilla-project.org).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of FileZilla.
...
[Releases]: https://github.com/jlesage/docker-filezilla/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/filezilla/tags
...
container, it would proxy all HTTP requests for `filezilla.domain.tld` to
...
server {
	...
	location /filezilla/ {
		...
		# Needed when audio support is enabled.
		location /filezilla/websockify-audio {
			proxy_pass http://docker-filezilla/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of FileZilla integrated into Docker image. |
...
Any desktop notification generated by FileZilla is forwarded to the
